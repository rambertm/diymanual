...
Func_RSGenerate(){
	...
	Xl.range(Xl.columns("H"),Xl.columns("S")).delete
	...
	While columnLabCount <= numEndColumn
	{
		...
		labCr := Func_GetLabData(Xl, rowCr, 0.5, 0.9, "[ Cr: ", columnLabCount)
		labAST := Func_GetLabData(Xl, rowAST, 8, 38, "[ AST: ", columnLabCount)
		labALT := Func_GetLabData(Xl, rowALT, 5, 43, "[ ALT: ", columnLabCount)
		...
		labCRP := Func_GetLabData(Xl, rowCRP, 0, 5.0, "[ CRP: ", columnLabCount)
	...
	while floorCount > 3
	{
		...
		while i <= numEndRow
		{
			...
			if ((roomNumber > currentFloor) and (roomNumber < nextFloor)){
				...
				if (currentRoomNumber <> beforeRoomNumber){
					if (beforeRoomNumber = 100){
						roundingRoomNumbers = %currentRoomNumber%
					}else{
						currentFloor := Floor(currentRoomNumber / 100)
						beforeFloor := Floor(beforeRoomNumber / 100)
						if (currentFloor <> beforeFloor){
							roundingRoomNumbers = %roundingRoomNumbers% / %currentRoomNumber%
						}else{
							roundingRoomNumbers = %roundingRoomNumbers%, %currentRoomNumber%
						}
					}
				}
				beforeRoomNumber := currentRoomNumber
			}
			i++
		}
		floorCount := floorCount - 1
